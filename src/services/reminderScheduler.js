        ...
        if (hoursElapsed >= 2 && hoursElapsed < 24) {
          ...
          if (!sentLog) {
            await sendPaymentReminder(client, '2h', 'Reminder: Complete Your Spain Visa Package Payment ✈️', 
              `Please complete your payment within today to initiate your Spain residency processing. Checkout link: ${process.env.FRONTEND_URL || 'http://localhost:5173'}/#/public/no-show-payment?clientId=${client.id}&amount=250`
            );
          }
        }
        ...
        if (hoursElapsed >= 24 && hoursElapsed < 48) {
          ...
          if (!sentLog) {
            await sendPaymentReminder(client, '24h', 'Action Required: Finish Your Spain Relocation Setup 🇪🇸',
              `It has been 24 hours. Don't lose access to your assigned specialist. Complete payment: ${process.env.FRONTEND_URL || 'http://localhost:5173'}/#/public/no-show-payment?clientId=${client.id}&amount=250`
            );
          }
        }
        ...
        if (hoursElapsed >= 48 && hoursElapsed < 120) {
          ...
          if (!sentLog) {
            await sendPaymentReminder(client, '2d', 'Reminder: Confirm Your Application Details & Invoice 🧾',
              `Your invoice is pending payment for 2 days. Final link: ${process.env.FRONTEND_URL || 'http://localhost:5173'}/#/public/no-show-payment?clientId=${client.id}&amount=250`
            );
          }
        }

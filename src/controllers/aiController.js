...
exports.getCeoBrief = async (req, res) => {
  try {
    ...
    let aiSummary = '';
    let aiSuggestions = [];
    let aiSource = 'fallback'; // Will be 'openai' if GPT is used
...
- The brief should sound premium, executive-focused, and address Wael personally. Include specific numbers where appropriate.`;
        ...
        if (response.data?.choices?.[0]?.message?.content) {
          ...
          aiSource = 'openai'; // Confirmed: OpenAI responded successfully
          console.log('[CEO Brief] Generated by OpenAI GPT successfully.');
        }
      } catch (err) {
...
• Customer feedback: **${feedbackCount} customer reviews and feedback** items require your review.`;
    ...
    console.log(`[CEO Brief] Final response source: ${aiSource}`);

    return res.status(200).json({
      success: true,
      aiSource,  // 'openai' or 'fallback' — check this in browser Network tab
      metrics: rawMetrics,
      brief: aiSummary,
